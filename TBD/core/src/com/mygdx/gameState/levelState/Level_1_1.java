...
//	private BitmapFont font = new BitmapFont();
	...
	private String levelDataDir = "/levelData/Level0/Level_1_data.xml";
	private String enemyDataDir = "/levelData/Level0/EnemyData_Level1.xml";

	
	public Level_1_1(LevelStateManager lsm) {
		super(lsm);
	...
	@Override
	public void init() {	
		lsm.setCurrentLevel(LevelStateManager.LEVEL1_1);
		
		MusicBox.play("YEH");
		MusicBox.loop("YEH", true);
//		getMap().dispose();
		setMap(new TmxMapLoader().load("map/level_2.tmx"));
...
//		enemyList = new Enemy[levelFileReader.getEnemyNum()];		
		...
		bob = new Bob(new Vector2(512, 64));
	...
	public void update(float dt) {
		if(!isPause){
			...
			if(isCompleted){
				//lsm.setState(LevelStateManager.ENDING);
...
//		cam.position.set(bob.getPositionScaled().x + bob.getDimension().x / 2 
//				   	   , bob.getPositionScaled().y + bob.getDimension().y / 2
//				       , 0);
