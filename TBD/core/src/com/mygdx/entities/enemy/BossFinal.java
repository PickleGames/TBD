...
public class BossFinal extends Enemy{
	private static Texture texture = new Texture("Image/Character/Enemy/final boss.png");
	public static TextureRegion textureRegion = new TextureRegion(texture);
	
//	private float timeElapsed;
//	private Random rand;
	
	public BossFinal(Vector2 position) {
		super(position);
		init();
	}
...
//		rand = new Random();
	...
	public void shoot(Vector2 target) {
		...
		for(Gun g: gun){
			if(Math.random() < .75f)((Pistol)g).shoot(target);
			else ((Pistol)g).shootSpecial();
		}		
	}
